...
class AuthRemoteRepository {
  ...
  // Tách logic đăng ký thành một hàm riêng để chạy trong isolate
  static Future<UserModel?> _signUpIsolate(Map<String, dynamic> data) async {
    try {
      print('Starting signup request with data: ${data.toString()}');
      ...
      if (res.statusCode != 201) {
        print('Signup error: ${res.body}');
        throw jsonDecode(res.body)['error'];
      }

      final user = UserModel.fromJson(res.body);
      print('Signup successful: ${user.toString()}');
      return user;
    } catch (e) {
      print('Exception in _signUpIsolate: $e');
      throw e.toString();
    }
  }
  ...
  Future<UserModel> signUp({
    ...
    try {
      ...
      final result = await compute(_signUpIsolate, {
        'name': name,
        'email': email,
        'password': password,
      });
